...
class User(Base):
    ...
    password_hash = Column(String(255), nullable=True)  # OAuth ユーザーは NULL
    is_active = Column(Integer, default=1)

    # OAuth フィールド
    oauth_provider = Column(String(50), nullable=True)  # "google" | "password" (NULL の場合ログイン不可)
    oauth_id = Column(String(255), nullable=True, index=True)  # Google sub
    name = Column(String(255), nullable=True)  # プロフィール名（Google から取得の場合）
